  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const uniqueYears = [...new Set(fetchedData.map(item => item.year))].sort((a, b) => b.localeCompare(a));
        ...
        <div className="w-full min-h-[50vh]">
          {loading ? (
            <div className="flex flex-row flex-wrap justify-center w-full gap-4">
               {[1, 2, 3, 4].map(i => (
                 <div key={i} className="w-[44%] sm:w-[28%] md:w-[20%] aspect-[3/4] bg-specialgrey/50 rounded-2xl animate-pulse mx-2 my-4"></div>
               ))}
            </div>
          ) : years.length === 0 ? (
