  ...
  return (
    <div className="Carousel-card m-[2px] [width:inherit] [height:inherit] top-0 left-0 right-0 bottom-0 flex flex-col justify-between rounded-xl shadow p-6 sm:pr-[2rem] space-y-3 space-x-0 sm:space-x-3 sm:space-y-0 sm:flex-row bg-[radial-gradient(circle_at_bottom_left,_var(--tw-gradient-stops))] from-orange-300 to-violet-300">
      <div className="my-auto sm:h-full sm:w-auto  w-full basis-[100px] grow-0 sm:basis-auto bottom -0 sm:grow">
             ...
             w-40 sm:h-full sm:w-full rounded-lg mx-auto my-auto sm:mx-none sm:object-cover" />
        </div>
        ...
        <div className="border-solid shadow-[1px] border-[2px] border-white-600 drop-shadow p-4 mt-0 ext-left rounded-lg sm:h-full sm:w-auto w-full font-['Inter']">
             <h3 className="Carousel-title block text-white-900 font-extrabold sm:text-5xl text-xl ">{Title}</h3>
            <h3 className="Carousel-designation block text-white-600 font-semibold sm:text-3xl text-lg  ">{Designation}</h3>
            <p className="Carousel-content block font-normal sm:text-5lg text-[1rem] mt-2 text-slate-100 sm:block ">{Content}</p>
        </div>
    </div>
  )
}
